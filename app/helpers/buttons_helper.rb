module ButtonsHelper
  ...
  def mark_as_deleted_button(model)
    form_with model: model, url: send("mark_as_deleted_#{model.class.to_s.downcase}_path", model), method: :post, html: { onsubmit: "return confirm(\"Are you sure you want to delete the following #{model.class.to_s.downcase}?\\n\\n#{model.summarize}\");"}, data: { turbo: false } do
      "<button class='btn-error'>
        #{ embedded_svg 'trashcan.svg', class: "w-5 h-5" }
        Delete
      </button>".html_safe
  ...
  def recover_button(model)
    form_with model: model, url: send("recover_#{model.class.to_s.downcase}_path", model), method: :post, data: { turbo: false } do
